...
export default function MusclePage() {
    ...
    return <>
        ...
        <div className="flex m-8 justify-between pr-5">
            <TextInput onChange={handleKeywordChange} icon={Search} type="text" placeholder="Search Muscles..."
                ...
                <TableBody className="divide-y">
                    {results.map((muscle) => (
                        <TableRow className="bg-white dark:border-gray-700 dark:bg-gray-800" key={muscle.id}>
                            ...
                            { editingMuscle === muscle && <TableCell>
                                <div className="flex ml-5 gap-10">
                                    <TextInput type="text" value={editingMuscleName} onChange={inputName} className="w-48" sizing="sm"></TextInput>
                                    <button><CircleCheck/></button>
                                    <button onClick={() => setEditingMuscle(undefined)}><CircleX/></button>
                                </div>
                            </TableCell>}
                            { editingMuscle !== muscle && <TableCell>
                                    {muscle.name}
